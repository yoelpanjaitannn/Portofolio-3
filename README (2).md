...
```
├── data/
│   └── UD_Sumber_Rejeki_AllData.xlsx
├── sql/
│   ├── level1_select_where.sql
│   ├── level2_join.sql
│   ├── level3_groupby_aggregasi.sql
│   └── level4_cte_window_analytics.sql
├── load_data.py
├── Executive_Summary_v4.docx
└── README.md
```
...
**Requirements:** Python 3.x, DBeaver

```bash
# 1. Install dependencies
pip install pandas openpyxl

# 2. Generate database dari Excel
py load_data.py
# Output: data/ud_sumber_rejeki.db (7 tabel, ~9.400 baris)

# 3. Buka DBeaver
# New Connection → SQLite → Browse → pilih file .db → Finish

# 4. Jalankan query dari folder sql/ secara berurutan
# Level 1 → 2 → 3 → 4
```

| Level | File | Konsep |
|-------|------|--------|
| 1 | `level1_select_where.sql` | SELECT, WHERE, LIKE, BETWEEN |
| 2 | `level2_join.sql` | INNER JOIN, LEFT JOIN |
| 3 | `level3_groupby_aggregasi.sql` | GROUP BY, HAVING, agregasi |
| 4 | `level4_cte_window_analytics.sql` | CTE, LAG, RANK, DSO/CEI/CCC |
...
- ✅ **Phase 2** — SQL Analysis & Database Setup (selesai)
- ⏳ **Phase 3** — Automated Early Warning System Dashboard (dalam pengerjaan)
...
🔗 [LinkedIn](https://www.linkedin.com/in/yoelpanjaitan) <!-- ganti dengan URL LinkedIn kamu yang benar -->
